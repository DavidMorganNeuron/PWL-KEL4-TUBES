...
@php
    $statusConfig = [
        'paid'      => ['bg' => '#DBEAFE', 'text' => '#1E40AF', 'dot' => '#2563EB', 'label' => 'Menunggu Diproses', 'pulse' => false, 'border' => '#BFDBFE'],
        'cooking'   => ['bg' => '#FEF3C7', 'text' => '#92400E', 'dot' => '#D97706', 'label' => 'Sedang Dimasak',    'pulse' => true,  'border' => '#FDE68A'],
        'completed' => ['bg' => '#D1FAE5', 'text' => '#065F46', 'dot' => '#059669', 'label' => 'Selesai',           'pulse' => false, 'border' => '#A7F3D0'],
        'canceled'  => ['bg' => '#FEE2E2', 'text' => '#991B1B', 'dot' => '#DC2626', 'label' => 'Dibatalkan',        'pulse' => false, 'border' => '#FECACA'],
    ];
...
>
    ...
    <div style="padding: 1rem 1.125rem 0.75rem; border-bottom: 1px solid #F0E8DC; display: flex; align-items: flex-start; justify-content: space-between; gap: 0.75rem;">
        ...
        <span
            ...
            style="display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px 3px 8px; border-radius: 9999px; font-size: 0.6875rem; font-weight: 600; letter-spacing: 0.02em; background-color: {{ $cfg['bg'] }}; color: {{ $cfg['text'] }}; border: 1px solid {{ $cfg['border'] }}; white-space: nowrap; flex-shrink: 0;"
        >
            <span
                aria-hidden="true"
                style="width: 6px; height: 6px; border-radius: 9999px; flex-shrink: 0; background-color: {{ $cfg['dot'] }}; {{ $cfg['pulse'] ? 'animation: kds-pulse 1.5s cubic-bezier(0.4,0,0.6,1) infinite;' : '' }}"
            ></span>
            {{ $cfg['label'] }}
        </span>
    </div>

    {{-- PESANAN: nama pelanggan + waktu masuk --}}
    ...
    <div style="padding: 0.75rem 1.125rem 1rem; border-top: 1px solid #F0E8DC; display: flex; align-items: center; justify-content: space-between; gap: 0.75rem;">
        ...
        <div style="display: flex; gap: 0.5rem; align-items: center;">
            ...
            @if($order->status === 'paid')
                {{-- form: paid → cooking --}}
                <form method="POST" action="{{ route('manager.orders.cook', $order->id_orders) }}" style="display:inline;">
                    @csrf
                    <button type="submit" class="pods-btn-primary kds-btn-cook" style="font-size: 0.8125rem; padding: 0.5rem 1rem; gap: 0.3rem;" aria-label="Mulai memasak pesanan {{ $order->order_number }}">
                ...
                {{-- form cancel --}}
                <form method="POST" action="{{ route('manager.orders.cancel', $order->id_orders) }}" class="kds-cancel-form" style="display:inline;">
                    @csrf
                    <input type="hidden" name="cancel_reason" class="kds-cancel-reason-input">
                    <button type="button" class="pods-btn-danger kds-btn-cancel" data-order-number="{{ $order->order_number }}" style="font-size: 0.8125rem; padding: 0.5rem 1rem; gap: 0.3rem;" aria-label="Batalkan pesanan {{ $order->order_number }}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" aria-hidden="true">
            ...
            @elseif($order->status === 'cooking')
                {{-- form: cooking → completed --}}
                <form method="POST" action="{{ route('manager.orders.done', $order->id_orders) }}" style="display:inline;">
                    @csrf
                    <button type="submit" class="pods-btn-primary kds-btn-done" style="font-size: 0.8125rem; padding: 0.5rem 1rem; gap: 0.3rem; background: #059669;" aria-label="Tandai selesai pesanan {{ $order->order_number }}" onmouseover="this.style.background='#047857'" onmouseout="this.style.background='#059669'">
                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" aria-hidden="true">
